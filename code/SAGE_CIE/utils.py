...
import argparse
import random
from sage_pyg import CausalSAGE
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='CausalSAGE', help='GNN model')  # GCN CausalGCN
    parser.add_argument('--bias_type', type=str, default='struc', help='bias_type, node or struc')
    parser.add_argument('--level', type=str, default='0.8', help='bias_grade, node(unbiased,0.7,0.8,0.9), struc(unbiased, 0.1, 0.2)')
    ...
    parser.add_argument('--idp', type=float, default=0.5, help='independent')  # context
    parser.add_argument('--co', type=float, default=0.5)
    # kernel
    parser.add_argument('--kernel', type=str, default='rbf', help='rbf, linear')
    # 最优模型准则crite
    ...
    # 特征
    parser.add_argument('--normal_feature', type=str2bool, default=False)
    # TensorboardX
    ...
    parser.add_argument('--train_print_steps', type=int, default=2)  # 不每一步都输出
...
def get_model(args):
    def model_func1(num_features, num_classes):
        return CausalSAGE(num_features, args.hidden, num_classes, args=args, dropout_pct=args.dropout)

    if args.model == 'CausalSAGE':
