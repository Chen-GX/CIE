...
def run_code(dataset, model, lr, weight_decay, epochs, dropout, idp, co, idp_type, crite, normal_feature):
    ...
    for bias_type in ['mixture']:  # , 'struc'
    ...
    for idp_t in idp_type:
        for cri in crite:
            for k in range(len(idp)):
                run_code(dataset, model, lr, weight_decay, epochs, dropout, idp[k], co[k], idp_t, cri, normal_feature)
