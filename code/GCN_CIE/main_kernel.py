...
def run_code(dataset, model, lr, weight_decay, epochs, dropout, idp, co, idp_type, crite, normal_feature, kernel):
    ...
    save_file_name = f'kernel_result_{dataset}_{idp}_{co}_{idp_type}_{crite}_{weight_decay}_{dropout}_{kernel}1-12'
...
if __name__ == '__main__':
    ...
    model = 'CausalGCN'
    lr = 0.01 # 0.015
    weight_decay = 1e-3 # 5e-4  # 记得给文件重命名
    epochs = 200
    dropout = 0.5  # 0.5
    idp = [0.5]  # 
    co = [0.5]  # 
        ...
        for cri in crite:
            for k in range(len(idp)):
                for kernel in kernels:
                    run_code(dataset, model, lr, weight_decay, epochs, dropout, idp[k], co[k], idp_t, cri, normal_feature, kernel)
