...
class CausalGCN(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim, args, dropout_pct) -> None:
        ...
        self.conv_feat = GCNConv(in_dim, hid_dim, gfn=False)  # 第一层的GCN
        ...
        self.context_convs = GCNConv(hid_dim, out_dim)
        self.objects_convs = GCNConv(hid_dim, out_dim)
        ...
        if self.args.cat_or_add == "cat":
            ...
            self.context_objects_convs = GCNConv(hid_dim, out_dim)
        else:
            assert False
