...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='CausalGCN', help='GNN model')  # GCN CausalGCN
    parser.add_argument('--bias_type', type=str, default='mixture', help='bias_type, node or struc or mixture')
    ...
    # 最优模型准则crite
    parser.add_argument('--crite', type=str, default='loss', help='acc/loss')
    # 独立的向量
...
def get_model(args):
    def model_func1(num_features, num_classes):
        return CausalGCN(num_features, args.hidden, num_classes, args=args, dropout_pct=args.dropout)

    if args.model == 'CausalGCN':
