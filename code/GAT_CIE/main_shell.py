...
def run_code(dataset, model, lr, weight_decay, epochs, dropout, idp, co, head1, head2, idp_type, crite):
    ...
    for idp_t in idp_type:
        for cri in crite:
            for k in range(len(idp)):
                run_code(dataset, model, lr, weight_decay, epochs, dropout, idp[k], co[k], head1, head2, idp_t, cri)
